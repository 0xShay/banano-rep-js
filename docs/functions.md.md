...
##### `getRepWeight(representativeAddress)`
returns raw delegated voting weight to `representativeAddress`

##### `getOnlineRepresentatives()`
returns an object of objects regarding every online representative and their respective raw voting weights

##### `getOnlineWeight()`
returns total raw online network voting weight

##### `repPercentage(representativeAddress)`
returns the % of online weight that `representativeAddress` has
